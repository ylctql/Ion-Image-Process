...
def _plot_summary(
    ...
    if names:
        ...
        ax.set_title("Total detected ions per group")
        fig.tight_layout()
    ...
    if all_y.size > 0:
        ...
        ax.set_ylabel("Count (all ions, all groups)")
        ax.set_title(f"Merged y histogram | N_ions={all_y.size}")
        fig.tight_layout()
    ...
    if all_y.size > 0 and all_group_idx.size == all_y.size:
        ...
        ax.set_xlabel("Group index (batch order)")
        ax.set_ylabel("y (pixel)")
        ax.set_title("Group index vs y (2D histogram)")
    ...
    if len(flat) >= 2:
        ...
        ax.set_xlabel("Frame index (flattened across groups)")
        ax.set_ylabel("Ion count (this frame)")
        ax.set_title(f"Ion count per frame | N={len(flat)} frames")
        if len(flat) <= 80:
            ax.set_xticks(xp)
